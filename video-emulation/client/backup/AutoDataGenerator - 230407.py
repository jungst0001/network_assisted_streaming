...
class AutoDataGeneratorOptions:
	def __init__(self):
		...
		###############################################
				# VM OPTION #
		self.LOCAL_MAX_PLAYER_PER_CLIENT = 2
		self.REMOTE_MAX_PLAYER_PER_CLIENT = 3
		###############################################
				# Auto OPTION #
		...
		if i in start_time_slot:
			for j in range(start_time_slot.count(i)):
				...
				if c < local_cListLen:
					...
					start_client_thread = Thread(target=startRemoteClient, args=(REMOTE_IP_LIST[c-local_cListLen], remoteHostHandler,))
					print('\033[95m' + f'start client!: {REMOTE_IP_LIST[c-local_cListLen]} at time {i}' + '\033[0m')
		...
		if i in stop_time_slot:
			for j in range(stop_time_slot.count(i)):
				...
				if c < local_cListLen:
					...
					stop_client_thread = Thread(target=stopRemoteClient, args=(REMOTE_IP_LIST[c-local_cListLen], remoteHostHandler,))
					print('\033[95m'+f'stop client!: {REMOTE_IP_LIST[c-local_cListLen]} at time {i + incomingTime + wait_time}'+'\033[0m')
